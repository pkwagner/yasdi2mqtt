...
bool mqtt_init(char *server, uint16_t port, char *ssl_cert, char *user, char *password, char *topic_prefix, int qos_level)
{
    ...
    sprintf(url, "%s://%s:%u", ssl_cert_global ? "ssl" : "tcp", server, port);
